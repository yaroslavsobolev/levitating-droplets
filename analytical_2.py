...
def find_B_to_match_curvature(target_curvature = 0.643, x0 = -33.25, B0=-0.321, xmax=20, W=0):
    ...
    root = optimize.brentq(fun, a=B0*1, b=B0*1.5)
    return root
...
def shape_for_W(W, B_prev, do_plot=True):
    B = find_B_to_match_curvature(W=W, B0=B_prev, target_curvature=upstream_curvature(W))
    x,y,curv = numerical_downstream(x0 = -33.25, B=B, xmax=20, W=W, do_plot=False)
    if do_plot:
...
W=0
x0 = -81
B = find_B_to_match_curvature(x0=x0, W=W, B0=-0.7, target_curvature=upstream_curvature(W))
