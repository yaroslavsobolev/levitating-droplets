...
def find_B_to_match_curvature(target_curvature = 0.643, x0 = -33.25, B_min=-0.323, B_max=-0.320, xmax=20, W=0):
    def fun(B):
        _, _, curvature_at_inf = numerical_downstream(x0 = x0, B=B, xmax=xmax, W=W, do_plot=False)
        return curvature_at_inf - target_curvature
    root = optimize.brentq(fun, a=B_min, b=B_max)
    return root
...
def make_upstream_curvature_plot(x0 = 20, do_plot=False):
    ...
    if do_plot:
        ...
        x,y,curv = numerical_upstream(x0=x0, A=1, xmin=-150, W=3, do_plot=True)
        print(curv)
        plt.show()
    curvs = []
    Ws = np.linspace(0, 5e-1*12, 30*12)
    for i,W in enumerate(Ws):
        x, y, curv = numerical_upstream(x0=x0, A=1, xmin=-150, W=W, do_plot=False)
        curvs.append(curv)
    np.save('analytical/ws.npy', Ws)
    np.save('analytical/curvs.npy', np.array(curvs))
    plt.plot(Ws, curvs, label='x0={0}'.format(x0))


# make_upstream_curvature_plot(x0=20)
# make_upstream_curvature_plot(x0=10)
# make_upstream_curvature_plot(x0=5)
# plt.legend()
# plt.show()
...
def shape_for_W(W, B_min, B_max, do_plot=True):
    x0 = -50
    xmax = 40
    B = find_B_to_match_curvature(x0=x0, W=W, B_min=B_min, B_max=B_max,
                                  target_curvature=upstream_curvature(W),
                                  xmax=xmax)
...
for i,W in enumerate(Ws):
    if i>10:
        expected_B = np.poly1d(np.polyfit(Ws[:i][-Nprev:], Bs[-Nprev:], 3))(W)
        B_min = (1 + delta_B)*expected_B
        B_max = (1 - delta_B)*expected_B
    else:
        expected_B = B_prev
        B_min = 1.3*B_prev
        B_max = 0.98*B_prev
    if i>15:
        delta_B = 0.002
    B_prev, Hmin = shape_for_W(W, B_max=B_max, B_min=B_min,
                               do_plot=(i % 10 == 0))
    Hmins.append(Hmin)
    Bs.append(B_prev)
...
np.save('analytical/downstream/Ws.npy', np.array(Ws))
np.save('analytical/downstream/Bs.npy', np.array(Bs))
np.save('analytical/downstream/Hmins.npy', np.array(Hmins))
plt.show()
