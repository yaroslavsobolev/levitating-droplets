...
target_folder = 'comsol_results/capillary_bridge_dynamics/sketch_2/'
...
def field_at_arbitrary_point(r0,z0,t0, precomputed=True):
    ...
    if not precomputed:
        ...
        a = interp_list[left_time_index][0](point)
        b = interp_list[left_time_index][1](point)
...
for r_here in rs[:250]:
    positions.append([r_here, 50.005])
...
for i in range(len(volume_list)):
    if volume_list[i] > 0:
        plt.loglog(1/tss[i], volume_list[i], 'o-', color='black')

plt.grid(b=True, which='major', color='#666666', linestyle='-')
plt.xlabel('Inverse time $1/ \\tau $, 1/s')
plt.ylabel('Transfer volume, fL')
plt.show()
