...
def process_one_folder(target_folder,
    ...
    if has_img_subfolders:
        img_subfolders = sorted(glob(video_folder + "img*/"))
    else:
        img_subfolders = [video_folder]
    ...
    for frame_id in range(start_frame, frame_id_max):
        ...
        ax_zoom = fig3.add_subplot(gs[1, 0])
        ax_zoom2 = fig3.add_subplot(gs[1, 1])
        ax_frame = fig3.add_subplot(gs[0, 1])
        # plt.subplots_adjust(wspace=0, hspace=0)

        for ax in [ax_zoom, ax_zoom2, ax_frame]:
            ax.set_axis_off()

        time_here = camera_frame_to_trace_time(frame_id)
        ax_scope.set_title('Time: {0:.5f} s'.format(time_here))
        ax_scope.plot(scope_time, scope_droplet,'o-')
        ...
        ax_frame.imshow(img_for_show, cmap='gray', vmin=18, vmax=500)
        # unsharp_mask(img_zoom(img_for_show - bkg_median), radius=1, amount=1, preserve_range=True),
        for_zoom = img_zoom(img_for_show - bkg_median, center=center)
        ...
        ax_zoom.imshow(for_zoom,
                       cmap='gray', vmin=-200, vmax=200, interpolation='spline16') #

        for_zoom = img_zoom(img_for_show - bkg_median, center=center2)
        fzmax = np.max(np.abs(for_zoom))
        # for_zoom = filters.median(for_zoom, np.ones((3, 3)))
        for_zoom = unsharp_mask(for_zoom, radius=1.5, amount=0.5, preserve_range=True)
        ax_zoom2.imshow(for_zoom,
                       cmap='gray', vmin=-200, vmax=200, interpolation='spline16') #
...
if __name__ == '__main__':
    ...
    #                    start_frame=0, fps=5000, dt=0.005,
    #                    center=(182, 94), size=30, time_scale=1,
    #                    has_img_subfolders=True)

    process_one_folder('E:/FAST_CAM/2021_May_10_SDN_printing/FASTS4_2021-05-10_SDN_15ms_width/',
                       scope_filename='data/5-10SDN-1ST-cleaned.csv',
                       start_frame=0, fps=None, dt=0.005,
                       center=(117, 64),
                       center2=(117, 149),
                       size=30, time_scale=1,
                       has_img_subfolders=False)
