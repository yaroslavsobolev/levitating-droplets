...
def get_coeff(main_reference_spectrum, target_spectrum, target_spectrum_wavelengths, slope, intercept, noise_std, reference_wavelengths,
    ...
    if not only_rh_reference:
        ...
        plt.plot(target_spectrum_wavelengths, target_spectrum, 'o', markersize=2, label='Measured spectrum')
...
def construct_reference(ref_file, ax):
    ...
    ax.loglog(slope_to_concentration_converter(xs), xs, label='Linear calibration')
    # plt.show()
...
def construct_reference_enhanced(ref_file, ax):
    ...
    slope_to_concentration_converter = interpolate.interp1d(calibration_slopes[:,1], calibration_slopes[:,0], fill_value='extrapolate')
    ax.loglog(calibration_slopes[:,0], calibration_slopes[:,1], 'o', label='Calibration points')
    ax.loglog(slope_to_concentration_converter(xs), xs, label='Calibration line')
    # plt.show()
    return wavelengths, baseline, noise_std, main_reference_spectrum, slope_to_concentration_converter
...
def get_concentration_from_spectrum(spectrum_file, spectrum_id=1,
        ...
        ref_file='fluorescence_data\\reference_rhodamine\\2021_may_18_NewREF_forSDN\\NewREF_Acquisition 1 2021-05-18 02_00_16.session__from558nm_2.txt',
    ...
    ax_calib.plot(concentration, slope, 'o', label='Example spectrum')
    ax_calib.set_xlabel('Dilution from 0.25 wt% solution of Rhodamine B in SLN')
    ax_calib.set_ylabel('Magnitude of Rhodamine B spectrum\nrelative to the spectrum at dilution factor $10^{-5}$')
    ax_calib.legend()
    f5 = plt.figure(5)
...
if __name__ == '__main__':
    ...
    root = tk.Tk()
    root.withdraw()
    # target_file = 'fluorescence_data/Jun_02_SDN_refocused/treated data_refocused/0137_SDN_refocused.treated_350_4000_9.16.txt'
    target_file = 'fluorescence_data/Jun_02_SDN_refocused/treated data_refocused/2309_SDN_refocused.treated_400_1000_1.36.txt'
