...
target_folder = 'E:/yankai_levitation_transfer_highspeed_videos/'\
                '2020_sep_25/'\
...
print(f'voltage max: {np.max(applied_voltage)}')
...
ax_scatter.set_ylabel('Applied potential, $\hat{U}$')
...
for i, crossing in enumerate(crossings):
    ...
    ax_scatter.scatter(time_within_period, crossing, s=30,
                       color=colors[i], alpha=0.2)
plt.tight_layout()
f5.savefig('figures/voltage_thresholds.png', dpi=600)
plt.show()